...
export function WhatsAppButton() {
  ...
  // The cart page adds its own fixed CTA bar in the same bottom-right corner on mobile —
  // showing both stacks two floating targets on top of each other, so this one steps aside there.
  const pathname = usePathname();
  const isCheckoutPage = pathname === "/cart";

  return (
    <div
      className={`fixed right-4 bottom-[calc(4rem+env(safe-area-inset-bottom)+1rem)] md:bottom-6 z-50 ${isCheckoutPage ? "hidden md:block" : "block"}`}
    >
      <span
        aria-hidden
        className="whatsapp-ping absolute inset-0 rounded-full bg-[#25D366]"
      />
      <a
        href={href}
        target="_blank"
        ...
        className="whatsapp-glow relative flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white transition-transform hover:scale-105 active:scale-95"
      >
